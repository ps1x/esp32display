...
#define AMPLITUDE 1300                             // Depending on your audio source level, you may need to alter this value. Can be used as a 'sensitivity' control.
#define AUDIO_IN_PIN 13                            // Signal in on this pin
...
#define MAX_MILLIAMPS 3000                         // Careful with the amount of power here if running off USB port
...
int randomGameChooser = 0;
// arduinoFFT FFT = arduinoFFT(vReal, vImag, SAMPLES, SAMPLING_FREQ);

// Button stuff
int buttonPushCounter = 0;
int mode = modes::SPECTRUM_MODE;
...
void changeMode()
{
  ...
  buttonPushCounter = (buttonPushCounter + 1) % 7;
}
...
void polar(int band, int power, int angle) {
  int x = power/2 * cos((band*360/16 + angle)*3.1415/180) + 7;
  int y = power/2 * sin((band*360/16 + angle)*3.1415/180) + 7;
  // Serial.println("band " + String(band*360/16) + "bar " + String(power/2));
  matrix->drawPixel(x,y, CHSV((band*360/16)*360/255, 255, 255));
}
...
void loop()
{
  ...
  if (buttonPushCounter != 5 && mode == modes::SPECTRUM_MODE)
    FastLED.clear();
  ...
  EVERY_N_SECONDS(1)
  {
    ...
    if (gameDelayer >= 5)
    {
      if (gameChosen == false)
      {
        randomGameChooser = random(200);
        gameChosen = true;
        mode = modes::GAME_MODE;
      }
    }
    else
    {
      mode = modes::SPECTRUM_MODE;
      gameChosen = false;
    }
    sumPower = 0;
  }
  ...
  switch (mode)
  {
  case modes::SPECTRUM_MODE:
    renderSpectrum();
    break;
  case modes::GAME_MODE:
    if (randomGameChooser >= 100)
    {
      renderSnakeGame();
    }
    else
